...
|---|---|---|
| **Fastly API key** (`FASTLY_API_KEY`) | env var / per-service config, read via [`get_fastly_api_key`](../../backend/config.py) | No documented procedure |
...
**Step 5 — Record the rotation.** Note the date, secret category, what was rotated, and what was checked — a short rotation-log entry kept wherever you track operational changes.

### 2.4 Storage hygiene (what NOT to do)

These are operational rules with code-level enforcement where possible:

- **Never put a secret in a URL query parameter.** It leaks to browser history, Referer headers, intermediaries, server access logs. The 2026-06 CDN-secret incident moved `cdn_secret` from query param to `x-fastly-key` header (in the [backend/routers/admin/](../../backend/routers/admin/) package).
