...
- Nothing creates the Postgres metadata schema at runtime: `metadata/base.py` wires `_init_schema` as the SQLite pool's `schema_fn` only, so SQLite self-initializes and Postgres does not. `scripts/setup_pg_schema.py` is a mandatory deploy step, not an optional runbook convenience — skip it and the stack boots clean, then fails every metadata query with `relation "cron_runs" does not exist`. Any table added by a SQLite migration rather than by `_SCHEMA` needs an explicit DDL entry in that script (`slow_queries` and `committed_buffers` both do).
