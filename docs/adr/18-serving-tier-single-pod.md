...
- **Chart deployment defaults CrashLoop.** `values.yaml` ships `config.ingestMode: celery` with `config.ducklakeCatalog: ""` and `secrets.metadataDsn: ""`, so a default `helm install` boots a backend that `validate_ingest_mode()` correctly refuses to start — now for two reasons rather than one. The gate is behaving as designed; the defaults are wrong. Deliberately out of scope here: fixing it means deciding whether the chart's default topology is single-pod/sync or celery-with-required-values, which is a chart decision, not a serving-tier one.

## Out of scope

- Implementing option 3 (or 2, or 4). Each needs its own ADR.
- The chart's `ingestMode`/`ducklakeCatalog` default mismatch, per above.
