...
| **Per-service metadata.db (SQLite)** | VM disk at `/mnt/app-data/data/services/{service_id}.metadata.db` | Lost on VM termination | Rebuildable from FOS via ingest re-scan + cron re-run |
| **Per-service DuckDB cache** | VM disk at `/mnt/app-data/data/services/{service_id}.duckdb` | Lost on VM termination | Disposable; regenerated by view-rebuild on first query |
| **Per-service Iceberg catalog SQLite** | VM disk at `/mnt/app-data/cache/{fos_bucket}/iceberg_catalog.db` (cache dir is keyed by FOS bucket, not service_id) | Lost on VM termination | Auto-refreshes from FOS metadata.json on stale-pointer detection |

There is no current backup process. An operator-local dev/test snapshot procedure exists (`tar` workflows for taking pre-deploy snapshots), but it is not a production backup story and is not exercised on prod data.
...
6. Rollup backfill runs via the [`POST /api/admin/backfill-bundle-rollups`](../../backend/routers/admin/compaction.py) endpoint (or the recompute cron); takes minutes-to-hours depending on per-service data volume.
...
2. Move the corrupted file out of the way: `mv data/services/{service_id}.metadata.db data/services/{service_id}.metadata.db.corrupted`.
...
1. Stop backend, delete `cache/{fos_bucket}/iceberg_catalog.db` (the cache dir is keyed by FOS bucket name, not service_id — map service_id → bucket via `configs/{service_id}.json` `bucket`), restart.
...
1. Stop backend, delete `data/services/{service_id}.duckdb`, restart.
...
- ~~**Service config off-VM backup.**~~ **Done 2026-06-10.** Operator-workstation script at [scripts/backup_service_configs.sh](../../scripts/backup_service_configs.sh) tars `/mnt/app-data/configs/` via `gcloud compute ssh + sudo tar` and uploads a timestamped tarball to a GCS bucket. Bucket has a 30-day delete lifecycle. Configuration (bucket, instance, zone, configs path) lives in env vars — no infra-specific values in the repo. Automation source (scheduling) is still operator's choice — operator-laptop cron, GH Actions workload-identity, or VM-side cron after SA attachment; the script itself works from any of the three. **Freshness is now observable (SRE-11):** when `BACKUP_MARKER_VM_PATH` is set, the script writes a VM-local marker on success that the admin System Health card reads (`config_backup` in `/api/admin/health-snapshot`) — so a never-run or stale backup is visible at a glance instead of silently rotting. The marker doesn't replace scheduling; it makes the absence of scheduling loud.
...
| Rollup backfill interrupted mid-run | Re-run it (admin backfill endpoint / recompute cron). Backfills are idempotent — they re-derive from the Iceberg source — so a partial run is safe to repeat. |
