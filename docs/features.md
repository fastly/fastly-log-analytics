...
### Insights
Automated anomaly detection that compares a recent window against a longer baseline to surface error spikes, regional traffic surges, new IPs, new geographic regions, WAF signal changes, cache efficiency collapses, cache eviction pressure, and latency regressions.
...
### Health probe
`GET /api/health` is a cheap liveness check. `GET /api/health?deep=1` also verifies per-service ingest freshness and returns 503 when any service is degraded (last ingest older than the configured `stale_minutes` or last sync errored). Safe to wire into a load balancer.
