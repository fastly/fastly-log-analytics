...
- **Default Schedule:** Evaluated every `rum_discovery_interval_secs` (configured via `rum.sync_interval_seconds` or falling back to `interval_seconds`, min: 5s).
...
- **Active-Request Politeness Gate:** Evaluates `should_defer_cron("rum_discovery", service_id)`. If active user/analyst queries are running on DuckDB, non-manual RUM discovery ticks defer to protect query latency and avoid lock contention.
- **Mutual Exclusion Rule:** Standard-mode `rum_discovery_{service_id}` must never be registered concurrently with the High-Scale `rum_discovery_{service_id}` worker path, to prevent duplicate ingestion into DuckLake `client_vitals` and `client_errors`.
...
1. **Prerequisite & Politeness Check:** Confirms `rum.enabled == true`. Checks `FLA_DEV_NO_CRONS=1`. If not a manual run, checks `should_defer_cron("rum_discovery", service_id)`.
2. **Progress Lifecycle Start:** Calls `start_cron_run(service_id, "rum_discovery")` returning `run_id`, then calls `start_progress(run_id, service_id=service_id, task="rum_discovery")`.
...
9. **Telemetry, Status & Log Recording:**
   - Uses the shared status rule: any failed record, quarantine capture, or FOS deletion
     marks the ingestion run `"error"`; a Faro-only issue is an additional `"warning"`.
   - Records the same zero-filled counters as request ingestion:
     `valid_records`, `malformed_records`, `corrupt_containers`,
     `quarantine_capture_failures`, `source_delete_failures`, `cap_evictions`,
     `objects_processed`, `objects_successful`, `objects_partial`, and `objects_failed`.
...
- **100% Query & API Call Capture:**
  - **FOS S3 Calls:** S3 operations must be recorded in `usage_log.db` under `cron.rum_discovery`.
...
- **Corrupt Beacon Payload:** Uses the same malformed-record handling as request logs:
  individual local quarantine items when possible, valid beacons preserved, and the run
  marked `"error"` for any failed record or evidence/deletion operation.
