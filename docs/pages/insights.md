...
- [ ] **3. Anomaly Evaluation Contract:** Verify all 5 categories return valid insight cards or clean passes. All cards should show up even if there are no anololies and even if the service does not have the required fields (in which case we should tell the user and direct them to enable the fields if they want).
