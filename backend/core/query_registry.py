...
- **Weak references to connections.** A strong ref would resurrect closed
  ...
  sqlite3.Connection does NOT (it has no ``__weakref__`` slot), so for
  SQLite the registry falls back to a strong-ref closure scoped to the
  ``ActiveQuery`` lifetime — benign because the caller already holds the
  connection for the duration of the query. See :func:`_safe_weakref`.
...
_SQL_TRUNCATE = 4096
_ERR_TRUNCATE = 512
# Bumped from the original 200 (2026-06-12) because SQLite cron noise was
...
# so 400 entries ≈ 200 KB resident. Cheap; the alternative (per-db_type
# ring buffers) was rejected as more complex without a measured win.
# SRE-18: this is the single source of truth for "how long a just-run
# query stays findable" — keep the frontend copy (page.tsx,
# useFilteredActive.ts) in sync with this value, not a stale larger number.
_HISTORY_CAP = 400
...
class QueryRegistry:
    ...
    def cancel_query(
        self, qid: int, *, admin_id: str | None = None
    ) -> Literal["cancelled", "not_found", "already_finished", "connection_gone"]:
    ...
    def snapshot(
        ...
        *,
        ...
        ``all_active=True`` overrides the since_seq filter for the active
        list (completed list is always since_seq-filtered). Live-monitor
        delta polling needs this: a still-running query with id below
        ``since_seq`` must still appear in every snapshot, otherwise the
        client's rolling buffer loses sight of long-running queries. The
        ``get_query(qid)`` helper still wants the strict since_seq filter
        on both lists, so the flag defaults to False.

        ``cold_limit`` caps the completed-history tail returned on cold
        loads (``since_seq == 0``). The full 400-row history is ~290 KB
        of JSON; the live-monitor UI virtualises and only renders a
        finite window, so on cold mount the page can take the latest N
        and backfill via since_seq deltas after. Ignored when
        ``since_seq > 0`` (subsequent delta polls always return
        everything newer regardless of cap).
        """
        now_mono = time.monotonic()
        active_rows: list[dict] = [
            _row_for_active(q, now_mono, full_sql)
            for q in list(self._queries.values())
            if all_active or q.query_id > since_seq
        ]
        completed_rows: list[dict] = []
        if include_completed:
            completed_rows = [_row_for_completed(c, full_sql) for c in list(self._history) if c.query_id > since_seq]
            if cold_limit is not None and cold_limit > 0 and since_seq == 0 and len(completed_rows) > cold_limit:
                # Keep the latest `cold_limit` rows (deque end). The
                # completed_rows list mirrors deque order (insert-order
                # oldest-first), so tail-slicing keeps the newest entries.
                completed_rows = completed_rows[-cold_limit:]
