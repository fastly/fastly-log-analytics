...
def _migration_021_ingest_ledger_fencing_and_hot_indexes(con: sqlite3.Connection) -> None:
    """Add fenced claim generations and indexes for ledger sweeps/finalization."""
    if not _has_table(con, "ingest_ledger"):
        return
    if not _has_column(con, "ingest_ledger", "lease_generation"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN lease_generation INTEGER NOT NULL DEFAULT 0")
    con.execute(
        "CREATE INDEX IF NOT EXISTS idx_ingest_ledger_claimed_scan ON ingest_ledger(service_id, status, claimed_at)"
    )
    con.execute(
        "CREATE INDEX IF NOT EXISTS idx_ingest_ledger_commit_scan "
        "ON ingest_ledger(service_id, status, committed_at, raw_deleted_at)"
    )


def _migration_022_ingest_ledger_publication_and_delete_claims(con: sqlite3.Connection) -> None:
    """Gate raw deletion on publication and persist its idempotent claim lease."""
    if not _has_table(con, "ingest_ledger"):
        return
    if not _has_column(con, "ingest_ledger", "published_at"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN published_at REAL")
    if not _has_column(con, "ingest_ledger", "raw_delete_claim_token"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN raw_delete_claim_token TEXT")
    if not _has_column(con, "ingest_ledger", "raw_delete_claimed_at"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN raw_delete_claimed_at REAL")
    con.execute(
        "CREATE INDEX IF NOT EXISTS idx_ingest_ledger_raw_delete_scan "
        "ON ingest_ledger(service_id, status, published_at, committed_at, raw_deleted_at, raw_delete_claimed_at)"
    )


def _migration_023_ingest_ledger_retry_schedule(con: sqlite3.Connection) -> None:
    """Persist retry classification and the next due conversion attempt."""
    if not _has_table(con, "ingest_ledger"):
        return
    if not _has_column(con, "ingest_ledger", "error_kind"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN error_kind TEXT")
    if not _has_column(con, "ingest_ledger", "next_attempt_at"):
        con.execute("ALTER TABLE ingest_ledger ADD COLUMN next_attempt_at REAL")
    con.execute(
        "CREATE INDEX IF NOT EXISTS idx_ingest_ledger_retry_scan ON ingest_ledger(service_id, status, next_attempt_at)"
    )
...
MIGRATIONS: dict[int, Callable[[sqlite3.Connection], None]] = {
    ...
    21: _migration_021_ingest_ledger_fencing_and_hot_indexes,
    22: _migration_022_ingest_ledger_publication_and_delete_claims,
    23: _migration_023_ingest_ledger_retry_schedule,
}
