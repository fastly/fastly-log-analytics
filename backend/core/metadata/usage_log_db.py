    ...
    try:
        ...
        try:
            ...
            if path not in _initialized:
                ...
                # Best-effort migration from the legacy metadata.db.
                # Idempotent — skips when the destination already has
                # rows OR the source is missing. Failures only log; the
                # write path is correct on a fresh empty db.
                try:
                    migrate_from_metadata_db(service_id, con)
                except Exception as e:
                    logger.warning(
                        "[usage_log_db] %s: migration from metadata.db failed: %s",
                        service_id,
                        e,
                    )
        except Exception:
...
def migrate_from_metadata_db(service_id: str, dest_con: sqlite3.Connection) -> int:
    """Idempotent one-shot copy of usage_log + usage_log_hourly_summary
    from the legacy metadata.db.

    Returns the number of usage_log rows copied (0 means already
    migrated OR no source rows exist). Strategy:

    - Skip silently if usage_log already has rows in the destination
      (migration ran on a prior boot, or this is a fresh service).
    - ATTACH the source metadata.db read-only, copy usage_log rows in
      batches keyed on the source's auto-increment id so a partial
      copy can resume cleanly on next boot.
    - Copy usage_log_hourly_summary directly (the AFTER INSERT trigger
      WILL fire during the usage_log copy and rebuild the summary from
      scratch — which is correct — but the explicit summary copy
      preserves any rows that were updated by reconcile_fastly_stats
      after the triggers' last fire, which the trigger replay won't
      reproduce). DELETE existing summary first (otherwise the trigger
      double-counts on top of the explicit copy).

    The legacy metadata.db keeps its usage_log table untouched as a
    rollback backstop for one release. The next release can drop it.
    """
    from backend.core.metadata.base import db_path as metadata_db_path

    # If destination already has data, we've migrated. Idempotency
    # backstop — also catches the "user manually ran the migration once"
    # case from a maintenance shell.
    existing = dest_con.execute("SELECT 1 FROM usage_log LIMIT 1").fetchone()
    if existing is not None:
        return 0

    src_path = metadata_db_path(service_id)
    if not os.path.exists(src_path):
        return 0

    # ATTACH the source read-only. If the source's usage_log table is
    # missing (very old install), there's nothing to copy.
    src_escaped = src_path.replace("'", "''")
    try:
        dest_con.execute(f"ATTACH DATABASE '{src_escaped}' AS legacy")
    except sqlite3.OperationalError as e:
        logger.warning("[usage_log_db] %s: legacy ATTACH failed: %s", service_id, e)
        return 0
    try:
        has_usage_log = dest_con.execute(
            "SELECT 1 FROM legacy.sqlite_master WHERE type='table' AND name='usage_log'"
        ).fetchone()
        if has_usage_log is None:
            return 0

        # Triggers in dest will fire as we INSERT and rebuild the
        # summary from scratch — that's correct for ingest-shaped rows.
        # We drop the summary first so any pre-existing default rows
        # don't survive (defensive — the destination should be empty
        # since the existence check above passed).
        dest_con.execute("DELETE FROM usage_log_hourly_summary")
        # Match the source's column projection explicitly so a future
        # column add on either side doesn't silently break the copy.
        dest_con.execute(
            """
            INSERT INTO usage_log
                (id, timestamp, service_id, operation_class, operation_type,
                 url, status, duration_ms, function_name, process_context, bytes, count)
            SELECT id, timestamp, service_id, operation_class, operation_type,
                   url, status, duration_ms, function_name, process_context, bytes, count
            FROM legacy.usage_log
            """
        )
        copied = dest_con.execute("SELECT COUNT(*) FROM usage_log").fetchone()[0]

        # Triggers maintain the summary as we insert, but
        # reconcile_fastly_stats may have UPDATEd summary rows in the
        # legacy db that the trigger replay won't reproduce. Overwrite
        # the trigger's summary with the legacy authoritative copy
        # where one exists.
        has_summary = dest_con.execute(
            "SELECT 1 FROM legacy.sqlite_master WHERE type='table' AND name='usage_log_hourly_summary'"
        ).fetchone()
        if has_summary is not None:
            ...
            dest_con.execute(
                ...
                INSERT INTO usage_log_hourly_summary
                    (service_id, hour, operation_class, operation_type, count, bytes, last_updated)
                SELECT service_id, hour, operation_class, operation_type, count, bytes, last_updated
                FROM legacy.usage_log_hourly_summary
                """
            )

        dest_con.commit()
        logger.info("[usage_log_db] %s: migrated %d usage_log rows from metadata.db", service_id, copied)
        return int(copied)
    finally:
        try:
            dest_con.execute("DETACH DATABASE legacy")
