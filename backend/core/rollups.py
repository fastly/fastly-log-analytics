...
def backfill_missing_hour_rollups(
    service_id: str, source: dict, lookback_hours: int = 24
) -> int:
    """Build per-hour rollups for any closed hour within ``lookback_hours``
    that has data in the base table but lacks a per-hour rollup file.

    Background. ``recompute_touched_hours`` skips the active (in-progress)
    hour during ingest because that hour is still being written. Normally
    the next sync tick after the hour closes catches it via a late-
    arriving file. But if all of an hour's data lands while that hour is
    still active (low-traffic services, tight delivery windows), no
    subsequent sync tick has new files for that hour — its per-hour
    rollup never gets built. The reader then falls back through to a
    live base-table scan for that hour (correct but slow) or, after the
    partial-day reader fix, undercounts top-N panels that need the
    missing hour.

    This function is the self-healing backstop. Cheap when there's
    nothing to do (one base-table DISTINCT + one listdir per service);
    rebuilds in a single batched COPY when there's a gap. Safe to call
    on every sync tick. Idempotent — hours that genuinely have no data
    are not in the "with data" set and so are not re-processed.

    Returns the number of missing hours that were rebuilt.
    """
    fields = _get_fields(source)
    if not fields:
        return 0
    ...
    # Use the first eligible field as a proxy for "does this hour have
    # any rollup at all?". recompute_touched_hours always batches all
    # fields together — so the presence of one field's hour-dir is a
    # reliable signal that the others are present too. Newly-added
    # fields without backfill markers are handled by
    # ``ensure_field_backfills`` separately, so they don't poison this
    # check.
    # If the rollup root itself doesn't exist yet (freshly-provisioned
    # service, or post-deploy on a service whose first sync hasn't
    # landed yet), every hour-with-data in the lookback window is
    # missing — the recompute_touched_hours call below creates the
    # tree on first write.
    rollup_root = _rollups_root(source)
    proxy_field = fields[0]
    proxy_dir = os.path.join(rollup_root, f"field={proxy_field}")
    existing_hours: set[str] = set()
    if os.path.isdir(proxy_dir):
        try:
            for entry in os.listdir(proxy_dir):
                if entry.startswith("hour="):
                    existing_hours.add(entry[len("hour=") :])
        except OSError:
            pass

    active_hour_dt = datetime.now(UTC).replace(minute=0, second=0, microsecond=0)
    earliest_dt = active_hour_dt - timedelta(hours=lookback_hours)

    # Find closed hours within the lookback window that have rows. The
    # active hour is intentionally excluded by the `< active_hour_dt`
    # bound — that hour is rolled up later, not here. Using a single
    # DISTINCT-strftime query bounds the cost to one scan per tick;
    # parquet partition pruning narrows it to the lookback range only.
    import duckdb
    ...
    try:
        try:
            rows = execute_with_stale_view_retry(
                ...
                source,
                lambda c: c.execute(
                    f"SELECT DISTINCT strftime(timestamp, '%Y-%m-%d-%H') "
                    f"FROM {table_ident} "
                    f"WHERE timestamp >= '{earliest_dt.isoformat()}' "
                    f"AND timestamp < '{active_hour_dt.isoformat()}'"
                ).fetchall(),
            )
        except duckdb.Error as e:
            logger.warning(
                "[rollups] %s: missing-hour backfill scan failed: %s", service_id, e
            )
            return 0
    finally:
        con.close()

    hours_with_data = {r[0] for r in rows if r[0]}
    missing = hours_with_data - existing_hours
    if not missing:
        return 0

    # recompute_touched_hours batches a single COPY across all missing
    # hours; the WHERE filter prunes to just those (strftime IN (...)).
    recompute_touched_hours(service_id, source, missing)
    return len(missing)
