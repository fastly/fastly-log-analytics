...
def _parse_key_layout_dt(key: str) -> tuple[str, datetime] | None:
    ...
    Returns the recognized layout and timestamp, or None for unrelated keys.
    """
    m = _V3_MINUTE_KEY_RE.search(key)
    if m:
        try:
            y, mo, d, h, mi = (int(g) for g in m.groups())
            return ("v3", datetime(y, mo, d, h, mi, tzinfo=UTC))
        except ValueError:
            return None
...
def list_fos_files(
    src: dict,
    prefix_subpath: str = "raw/request/",
...
def ingest(
    ...
    Downloads .gz request log files from the raw/request/ prefix, parses JSON, casts types,
...
LEDGER_RETRY_BASE_S = 30
LEDGER_RETRY_MAX_S = 60 * 60
LEDGER_RETRY_JITTER_RATIO = 0.25
...
def _ledger_record_failure(
    ...
    generation_clause = ""
    row = con.execute(
        "SELECT attempts, status, lease_generation FROM ingest_ledger WHERE service_id=? AND object_key=?",
        (service_id, object_key),
    ).fetchone()
    if row is None:
        return "unknown"
    attempts = int(row[0] or 0)
    if expected_generation is not None and int(row[2] or 0) != expected_generation:
        return "stale"
    next_attempts = attempts + 1
    terminal = next_attempts >= LEDGER_MAX_ATTEMPTS
    next_attempt_at = None
    if not terminal:
        delay = min(LEDGER_RETRY_MAX_S, LEDGER_RETRY_BASE_S * (2 ** max(0, next_attempts - 1)))
        jitter = random.uniform(0, delay * LEDGER_RETRY_JITTER_RATIO)
        next_attempt_at = time.time() + delay + jitter
    generation_clause = ""
    params: tuple = (error[:500], error_kind, next_attempt_at, LEDGER_MAX_ATTEMPTS, service_id, object_key)
    if expected_generation is not None:
        generation_clause = " AND status = 'claimed' AND lease_generation = ?"
        params += (expected_generation,)
    cur.execute(
        "UPDATE ingest_ledger SET attempts = attempts + 1, last_error = ?, error_kind=?, next_attempt_at=?, "
        "status = CASE WHEN attempts + 1 >= ? THEN 'quarantined' ELSE 'discovered' END, "
        "claimed_by = NULL, claimed_at = NULL "
        "WHERE service_id = ? AND object_key = ?" + generation_clause,
        params,
    )
    if cur.rowcount == 0 and expected_generation is not None:
        return "stale"
    con.commit()
    result = con.execute(
    ...
    if status == "quarantined":
        logger.error(
            "[ledger] %s: %s QUARANTINED after %s failed convert attempts: %s",
            service_id,
            object_key,
            result[1],
            error,
        ...
        logger.warning(
            "[ledger] %s: convert failed for %s (attempt %s; retry at %.3f): %s",
            service_id,
            object_key,
            result[1],
            next_attempt_at or 0,
            error,
        )
    return status


def _ledger_mark_committed(con, service_id: str, object_key: str, expected_generation: int) -> str:
    """Fence the terminal success transition to the worker's lease generation."""
    cur = con.cursor()
    cur.execute(
        "UPDATE ingest_ledger SET status='committed', committed_at=? "
        "WHERE service_id=? AND object_key=? AND status='claimed' AND lease_generation=?",
        (time.time(), service_id, object_key, expected_generation),
    )
    con.commit()
    return "committed" if cur.rowcount else "stale"
...
def convert_object(service_id: str, object_key: str, worker_id: str) -> str:
    ...
    cur.execute(
        "UPDATE ingest_ledger SET status='claimed', claimed_by=?, claimed_at=?, next_attempt_at=NULL, "
    ...
    lease_generation = int(claim_row[0]) if claim_row is not None else None
    con_meta.commit()
    if not claimed:
        return "not_claimed"
                ...
                try:
                    ...
                    if "404" in str(head_err) or "Not Found" in str(head_err) or "NoSuchKey" in str(head_err):
                        cur.execute(
                            ...
                            (service_id, object_key, lease_generation),
                        )
                        con_meta.commit()
...
def _ledger_claim_batch(con, service_id: str, object_keys: list[str], worker_id: str) -> dict[str, int]:
    ...
    for i in range(0, len(object_keys), _LEDGER_KEY_SLICE):
        ...
        cur.execute(
            ...
            "RETURNING object_key, lease_generation",
            (worker_id, now, service_id, *key_slice),
        )
        won.update({r[0]: int(r[1]) for r in cur.fetchall()})
    con.commit()
    return won
    ...
    if src is None:
        for object_key in claimed_keys:
            _ledger_record_failure(
                ...
                service_id,
                object_key,
                "no source registered for service",
...
def sweep_ledger_once(service_id: str, lookback_hours: int = 4) -> dict:
    ...
    rum_like_pattern = f"{prefix_path}/raw/rum/%" if prefix_path else "raw/rum/%"

    cur.execute(
        "UPDATE ingest_ledger SET status='discovered', claimed_by=NULL, claimed_at=NULL, next_attempt_at=? "
        "WHERE service_id=? AND status='claimed' AND claimed_at < ? AND object_key NOT LIKE ? "
        "RETURNING object_key",
        (now, service_id, now - LEDGER_RECLAIM_AFTER_S, rum_like_pattern),
    )
    ...
    stuck = [
        ...
        for r in con.execute(
            ...
            (service_id, rum_like_pattern, now, stale_cutoff),
        ).fetchall()
    ...
    pending_batches = math.ceil(pending / LEDGER_CONVERT_BATCH_SIZE) if pending else 0
    redispatched = 0
    if pending:
        broker_ok = False
        try:
            ...
            queue_depth = 0
        if broker_ok and queue_depth < pending_batches:
            ...
            # exists to remove. The raw/rum/ exclusion lives on the queries
            ...
            logger.info(
                "[ledger] %s: %d pending object(s) require %d batch message(s), "
                "but q.ingest already holds %d message(s) — skipping re-dispatch",
                service_id,
                pending,
                pending_batches,
                queue_depth,
            ...
            logger.warning(
                "[ledger] %s: q.ingest depth unavailable; skipping redispatch of %d pending object(s)",
                service_id,
                pending,
            )
    ...
    try:
        dead_letter_row = con.execute(
            "SELECT COUNT(*) FROM ingest_ledger WHERE service_id=? AND status IN ('quarantined', 'dead_letter')",
            (service_id,),
        ).fetchone()
...
# SAME ``ingest_ledger`` table — RUM object keys live under ``raw/rum/...``
# and regular-log keys under ``raw/request/...`` (see backend/core/rum_ingest.py's
# ``ingest_rum_logs``, which LISTs ``prefix_subpath="raw/rum/"``), so the two
...
def discover_rum_prefix(service_id: str, prefix_subpath: str | None = None, start_time: str | None = None) -> int:
    ...
    ``prefix_subpath`` defaults to the RUM root (``raw/rum/``) rather than
    relying on ``list_fos_files``'s own default (``raw/request/``, the regular-log
    ...
    list_kwargs: dict = {"start_time": start_time, "prefix_subpath": prefix_subpath or "raw/rum/"}
    fos_result = _drain_generator(list_fos_files(src, **list_kwargs))
    if not fos_result:
...
def _parse_rum_beacon_file(local_path: str, service_id: str) -> tuple[list[dict], list[dict], list[tuple[str, str]]]:
    """Decode+parse one ``raw/rum/*.gz`` beacon file.
...
def convert_rum_object(service_id: str, object_key: str, worker_id: str) -> str:
    """RUM counterpart of ``convert_object``: claim one ``raw/rum/`` ledger
...
def sweep_rum_ledger_once(service_id: str, lookback_hours: int = 4) -> dict:
    ...
    src = get_source_for_service(service_id)
    prefix_path = (src.get("prefix", "") if src else "").strip("/")
    rum_prefix = f"{prefix_path}/raw/rum/" if prefix_path else "raw/rum/"
    like_pattern = rum_prefix + "%"

    cur.execute(
        "UPDATE ingest_ledger SET status='discovered', claimed_by=NULL, claimed_at=NULL, next_attempt_at=? "
        "WHERE service_id=? AND status='claimed' AND claimed_at < ? AND object_key LIKE ? RETURNING object_key",
        (now, service_id, now - LEDGER_RECLAIM_AFTER_S, like_pattern),
    )
    reclaimed = [r[0] for r in cur.fetchall()]
    con.commit()
    ...
    stuck = [
        ...
        for r in con.execute(
            ...
            "AND (next_attempt_at IS NULL OR next_attempt_at <= ?) "
            "AND (claimed_at IS NULL OR claimed_at < ?) LIMIT 5000",
            (service_id, like_pattern, now, stale_cutoff),
        ).fetchall()
        if r[0] not in reclaimed
    ]
    ...
    if pending:
        ...
        if broker_ok and queue_depth < pending_messages:
            ...
            if not broker_ok:
                logger.warning(
                    "[ledger] %s: RUM q.ingest depth unavailable; skipping redispatch of %d pending object(s)",
                    service_id,
                    pending,
                )
            else:
                logger.info(
                    "[ledger] %s: %d RUM pending object(s) require %d message(s), "
                    "but q.ingest already holds %d message(s) — skipping re-dispatch",
                    service_id,
                    pending,
                    pending_messages,
...
RAW_DELETE_CLAIM_LEASE_S = 15 * 60


def _mark_ledger_published(service_id: str, *, rum: bool = False) -> int:
    """Record that DuckLake has durably published the committed ledger rows."""
    from backend.core.metadata.base import get_con

    con = get_con(service_id)
    prefix_clause = "object_key LIKE ?" if rum else "object_key NOT LIKE ?"
    cur = con.cursor()
    cur.execute(
        "UPDATE ingest_ledger SET published_at=? "
        "WHERE service_id=? AND status='committed' AND published_at IS NULL AND " + prefix_clause,
        (time.time(), service_id, "rum/%"),
    )
    con.commit()
    return cur.rowcount
...
def finalize_committed_raw(service_id: str, batch_size: int = 10_000) -> dict:
    ...
    ``provisioning.cron_sync.delete_after`` (default True), and stands down
    unconditionally when ``provisioning.cron_sync.high_scale_shared_source``
    is set — see ``backend.config.resolve_raw_delete_after``. Idempotent:
    ...
    eligible_query = (
        "SELECT object_key FROM ingest_ledger "
        "WHERE service_id = ? AND status = 'committed' AND published_at IS NOT NULL "
        "AND raw_deleted_at IS NULL AND committed_at < ? "
        "AND (raw_delete_claim_token IS NULL OR raw_delete_claimed_at < ?) LIMIT ?"
    )
    claim_expired = time.time() - RAW_DELETE_CLAIM_LEASE_S
    eligible = [r[0] for r in con.execute(eligible_query, (service_id, cutoff, claim_expired, batch_size)).fetchall()]
    ...
    for key in eligible:
        cur.execute(
            "UPDATE ingest_ledger SET raw_delete_claim_token=?, raw_delete_claimed_at=? "
            "WHERE service_id=? AND object_key=? AND status='committed' "
            "AND published_at IS NOT NULL AND raw_deleted_at IS NULL "
            "AND committed_at < ? "
            "AND (raw_delete_claim_token IS NULL OR raw_delete_claimed_at < ?)",
            (claim_token, claimed_at, service_id, key, cutoff, claim_expired),
        )
        if cur.rowcount:
            claimed.append(key)
    con.commit()
    eligible = claimed
    if not eligible:
        return {"deleted": 0, "eligible": 0, "delete_after": True}
    ...
    for i in range(0, len(eligible), 1000):
        ...
        for key in chunk:
            if key in failed_keys:
                cur.execute(
                    "UPDATE ingest_ledger SET raw_delete_claim_token=NULL, raw_delete_claimed_at=NULL "
            ...
            cur.execute(
                ...
                "raw_delete_claim_token=NULL, raw_delete_claimed_at=NULL "
                "WHERE service_id=? AND object_key=? AND raw_delete_claim_token=? AND raw_deleted_at IS NULL",
                (now, service_id, key, claim_token),
            )
