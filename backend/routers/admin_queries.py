...
router = APIRouter(prefix="/api/admin", tags=["admin", "query-monitor"], responses=DEFAULT_ERROR_RESPONSES)
...
def _ensure_enabled() -> None:
    if not _enabled():
        ...
        raise HTTPException(status_code=404, detail=not_found("query_monitor_disabled"))
...
def list_queries(
    ...
    cold_limit: int | None = Query(None, ge=1, le=400),
) -> SnapshotResponse:
    _ensure_enabled()
    # ``all_active=True`` keeps long-running queries visible across delta
    # polls — without it, an active query whose id is below the client's
    # lastSeqRef would drop out of subsequent responses even though it's
    # still running, and the live-monitor UI would lose sight of it.
    # Completed rows stay since_seq-filtered (that's where the wire-byte
    # win comes from on busy services).
    # ``cold_limit`` caps the completed-history tail on the very first
    # mount fetch (since_seq=0). The UI virtualises and only paints a
    # few dozen rows; full 400-row dump on every cold load was 290 KB
    # of JSON parsed in the browser for output it never showed.
...
def count_persisted_slow_queries(
    ...
    _ensure_enabled()
    if not service_id:
        raise HTTPException(status_code=422, detail=make_error("service_id_required"))
...
def get_query(qid: int) -> dict[str, Any]:
    ...
    if active is None:
        raise HTTPException(status_code=404, detail=not_found("query_not_found"))
...
def cancel_query(qid: int, request: Request) -> CancelResponse:
    ...
    if not _check_cancel_rate(admin_id):
        raise HTTPException(
            status_code=429,
            detail=make_error("cancel_rate_limit_exceeded"),
...
# R-1: drain the per-IP cancel rate-limit deque between tests.
from backend.utils.cache_registry import CacheRegistry as _CacheRegistry  # noqa: E402

_CacheRegistry.register("routers.admin_queries._cancel_history", _cancel_history)
