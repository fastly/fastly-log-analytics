...
async def sync_status_stream(
    ...
    """Push the cached sync-status snapshot to the browser whenever the
    sync cron refreshes it.

    Replaces the per-tab 30s poll of ``GET /api/sync-status`` for the
    header badge. The first event delivered is the *current* cached
    snapshot, so a freshly mounted page renders immediately without
    waiting up to one cron tick. Subsequent events are pushed by
    ``sync_status_publisher.publish()`` (called from
    ``backend/cron/jobs/sync.py`` after each ``refresh_config_status``).

    Path-prefix-blocked for analysts automatically — see
    ``_ANALYST_BLOCKED_SUBPATHS`` ("/api/sync-status") in
    ``backend/utils/remote_access.py``, which matches with a trailing-/
    rule so ``/api/sync-status/stream`` inherits the gate.
    """
    if not service_id:
        raise HTTPException(status_code=422, detail=make_error("x_service_id_required"))

    async def stream() -> AsyncIterator[str]:
        # Send the current cached snapshot immediately so the badge
        # doesn't blank-flash on connect. compute_sync_status_cached
        # returns None if no cached status has been persisted yet (fresh
        # service / cold start) — in that case the bootstrap-seeded
        # React Query cache holds the badge until the first cron tick.
        # Wrap in asyncio.to_thread so the disk-I/O fall-through inside
        # compute_sync_status_cached doesn't stall the event loop.
