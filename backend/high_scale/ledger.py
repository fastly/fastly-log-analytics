...
class HighScaleLedger:
    ...
    def mark_source_missing(self, service_id: str, object_key: str, *, error: str) -> None:
        """Record a claimed source object as durably, permanently missing.

        Used when a read confirms the raw object is gone from storage (e.g.
        deleted by a legacy raw-deletion path racing high-scale ownership).
        This is a terminal state distinct from a transient failure: it must
        be recorded for operator review, not silently retried forever and
        not treated as archived/acknowledged.
        """
        if not error:
            raise ValueError("an error description is required to mark a source missing")
        row = self._con.execute(
            "SELECT status FROM source_objects WHERE service_id=? AND object_key=?",
            (service_id, object_key),
        ).fetchone()
        if row is None:
            raise KeyError(object_key)
        if row["status"] in {"acknowledged", "source_deleted"}:
            raise ArchiveNotVerified(f"cannot mark a durably archived source missing: {object_key}")
        if row["status"] == "source_missing":
            return
        self._con.execute(
            "UPDATE source_objects SET status='source_missing',last_error=?,lease_until=NULL "
            "WHERE service_id=? AND object_key=?",
            (error, service_id, object_key),
        )
        self._con.commit()
    ...
    def expired_claims(self, service_id: str, domain: str, *, limit: int) -> tuple[SourceObject, ...]:
        if limit <= 0:
            raise ValueError("limit must be positive")
        rows = self._con.execute(
            "SELECT * FROM source_objects "
            "WHERE service_id=? AND domain=? AND status='claimed' "
            "AND lease_until IS NOT NULL AND lease_until <= ? "
            "ORDER BY lease_until, object_key LIMIT ?",
            (service_id, domain, time.time(), limit),
        ).fetchall()
