...
# Mirrors backend.high_scale.source_discovery.INITIAL_SOURCE_CURSOR (not
# imported directly: source_discovery imports this module, so importing
# back would cycle). A domain that has never been given its own cursor row
# must start from its own beginning, never from another domain's cursor —
# see the comment on source_cursor_for below for why.
_INITIAL_SOURCE_CURSOR = "__initial__"
...
class OwnershipStore:
    def __init__(self, database: str = ":memory:") -> None:
        ...
        self._con.execute(
            """
            CREATE TABLE IF NOT EXISTS service_ownership_cursors (
        ...
        *,
        ...
        try:
            ...
            if domain is None:
                ...
                self._con.execute(
                    """
                    INSERT INTO service_ownership_cursors (service_id, domain, source_cursor)
                    VALUES (?, ?, ?)
                    ON CONFLICT(service_id, domain) DO UPDATE SET source_cursor=excluded.source_cursor
                    """,
    ...
    def source_cursor_for(self, service_id: str, domain: str) -> str:
        ...
        error). A domain with no cursor of its own must start from its own
        beginning; the idempotent discover/claim dedup makes that safe.
        """
        row = self._con.execute(
            "SELECT source_cursor FROM service_ownership_cursors WHERE service_id=? AND domain=?",
            (service_id, domain),
        ).fetchone()
        return str(row["source_cursor"]) if row is not None else _INITIAL_SOURCE_CURSOR
