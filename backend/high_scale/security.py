...
def security_aggregates(
    ...
    ngwaf_bots_rows = _dimension_rows(
        ...
        end_time=end_time,
        select="value AS bot_name, max(wellknown_bot_name) AS wellknown_bot_name, max(bot_category) AS category, sum(requests) AS request_count",
        group_by="value",
...
def top_bots(
    ...
    ngwaf_bots_rows = _dimension_rows(
        ...
        start_time=start_time,
        end_time=end_time,
        select="value AS bot_name, max(wellknown_bot_name) AS wellknown_bot_name, max(bot_category) AS category, sum(requests) AS request_count, sum(verified_count) AS verified, sum(impersonator_count) AS impersonator",
        group_by="value",
        order_by="request_count DESC",
        limit=10,
    )
