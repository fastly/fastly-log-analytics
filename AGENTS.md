...
3. **`ReportLayout`** for analytics pages — bundles `useActiveService/useTimeRange/useTimezone + useReportConfig + useDebouncedFilterPayload + useUrlFilterSync + useServiceQuery + ChartIntervalButtons + ReportShell`. Fall back to `ReportShell` only for multi-query or non-standard chrome pages.
