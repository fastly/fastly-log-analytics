...
- **10.14** **Final CI gate:** `--cov-fail-under` at 85% backend, `coverage.thresholds.lines` at 58% frontend. Load-harness CI step green. Security-regression count ≥ baseline. mypy strict on the touched-module list. ⚠️ **Partial 2026-06-12.** Frontend gate ratcheted 53 → 55 (current actual 57.12%, up from 55.19%) after covering `lib/toast.ts`, `lib/api/custom-fields.ts`, and `lib/workers/parseJson.ts`. Still ~36 covered lines short of the 58% target. Backend gate, load-harness emitter, and mypy strict ratchet remain pending. See [v2_tag_readiness.md](v2_tag_readiness.md) for the live status table and next-step recommendations.
