...
- **10.13** **Final mypy sweep:** every backend module either mypy-strict or explicitly justified in `pyproject.toml` overrides with a comment. Goal: `ignore_errors` list ≤ 3 modules max. ⚠️ **Partial 2026-06-11 → 2026-06-12.** Replaced the five wildcard patterns (`backend.routers.*`, `backend.utils.*`, etc.) with an explicit per-file list — 36 → 22 entries after two 2026-06-12 burndown waves (cleared 14 modules total covering the zero-error and single-error tiers: `log_fields`, `sqlite_profiler`, `sql_validator`, `vcl_validator`, `core/duckdb`, `routers/dashboard`, `models/lake`, `utils/telemetry`, `utils/bot_sources`, `utils/terraform_gen`, `provision/session_scoring_orchestrator`, `routers/bootstrap`, `provision/fos_setup`, `repositories/insights/definitions`). Net effect: ~93 files that were silently swept up by the wildcards now get real mypy coverage. **Still pending:** 22 explicit entries need further burndown to reach the ≤ 3 target — the remaining modules each have 2-13 actual errors and need real type narrowing (especially `repositories/origin` with 13 `Any \| None` indexing errors). See the override block in `pyproject.toml` for the canonical list + refresh procedure.
- **10.14** **Final CI gate:** `--cov-fail-under` at 85% backend, `coverage.thresholds.lines` at 58% frontend. Load-harness CI step green. Security-regression count ≥ baseline. mypy strict on the touched-module list. ⚠️ **Partial 2026-06-12.** **Frontend gate ✅ shipped at the final 58% target** (actual 61.66%, up from 55.19%) after covering `lib/toast.ts`, `lib/api/custom-fields.ts`, `lib/workers/parseJson.ts`, and three `components/ProvisionWizard/wizard-*` helpers. Backend gate (currently 80, actual 82, target 85), load-harness emitter, and mypy strict ratchet remain pending. See [v2_tag_readiness.md](v2_tag_readiness.md) for the live status table and next-step recommendations.
