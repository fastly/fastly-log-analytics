...
- **10.13** **Final mypy sweep:** every backend module either mypy-strict or explicitly justified in `pyproject.toml` overrides with a comment. Goal: `ignore_errors` list ≤ 3 modules max. ✅ **DONE 2026-06-12 — list at 0 entries** (target exceeded; user reset to zero technical debt). Five burndown waves cleared 36 modules total: zero-error, single-error, 1-2-error, 3-error, and 5-13-error tiers. Net effect: ~93 files that were silently swept up by the original wildcards now type-check under default mypy settings; every backend module is now covered. The `[[tool.mypy.overrides]]` block stays in pyproject.toml as a documented empty placeholder (with a "last resort only" comment) so a future regression can't accidentally re-introduce wildcard suppression.
- **10.14** **Final CI gate:** `--cov-fail-under` at 85% backend, `coverage.thresholds.lines` at 58% frontend. Load-harness CI step green. Security-regression count ≥ baseline. mypy strict on the touched-module list. ⚠️ **Partial 2026-06-12.** **Frontend gate ✅ shipped at the final 58% target** (actual 61.66%, up from 55.19%). **Load-harness CI step ✅ shipped** — `scripts/emit_perf_latest.py` runs a 100K-row synthetic DuckDB workload in ~2 s wall, `scripts/perf_gate.sh` fails on >50 % regression vs `tests/perf/baseline.json`. The 36 M-row production targets stay documented in `baseline.json` `production_targets_comment` and are validated by the manual `scripts/dev/loadtest_probe.sh`, not the CI gate. **Backend gate ratcheted 80 → 82** (actual 83 %, up from 82 %) after coverage waves for `metadata/reconciliation`, `cron/jobs/compaction`, `repositories/session_scoring`, `core/data_migrations`, `utils/tunnel/state`, `routers/dashboard`, `repositories/views`, `utils/sqlite_profiler`. Path to gate=85 needs ~500 more covered lines on the heavyweight modules (`rollups.py`, `routers/admin.py`, `_duckdb_status.py`, `core/ingest.py`). **mypy-strict ratchet started**: 8 modules opted in (live-query-monitor surface + this session's tested set). See [v2_tag_readiness.md](v2_tag_readiness.md) for the live status table and next-step recommendations.
