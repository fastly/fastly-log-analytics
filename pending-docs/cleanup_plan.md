...
- **10.13** **Final mypy sweep:** every backend module either mypy-strict or explicitly justified in `pyproject.toml` overrides with a comment. Goal: `ignore_errors` list ≤ 3 modules max. ⚠️ **Partial 2026-06-11 → 2026-06-12.** Replaced the five wildcard patterns (`backend.routers.*`, `backend.utils.*`, etc.) with an explicit per-file list — 36 → 32 entries after the 2026-06-12 burndown wave (cleared `core/log_fields`, `utils/sqlite_profiler`, `utils/sql_validator` via `NoReturn` annotation, and `utils/vcl_validator` via swapping builtin `callable` for `typing.Callable`). Net effect across both waves: ~93 files that were silently swept up by the wildcards now get real mypy coverage. **Still pending:** 32 explicit entries need further burndown to reach the ≤ 3 target. Each is a per-module fix-or-annotate. See the override block in `pyproject.toml` for the canonical list + refresh procedure.
