...
- **10.13** **Final mypy sweep:** every backend module either mypy-strict or explicitly justified in `pyproject.toml` overrides with a comment. Goal: `ignore_errors` list ≤ 3 modules max. ⚠️ **Partial 2026-06-11 → 2026-06-12.** Replaced the five wildcard patterns (`backend.routers.*`, `backend.utils.*`, etc.) with an explicit per-file list — 36 → 10 entries after four 2026-06-12 burndown waves clearing 26 modules total (zero / single / 1-2 / 3-error tiers). Wave-4 cleared the 3-error tier: `config`, `deps`, `repositories/_base`, `repositories/sessions`, `routers/share_auth`, plus `repositories/insights/registry` (annotation-unchecked note). Net effect: ~93 files that were silently swept up by the wildcards now get real mypy coverage. **Still pending:** 10 explicit entries need further burndown to reach the ≤ 3 target — the remaining modules each have 5-13 actual errors and need real type narrowing (especially `repositories/origin` with 13 `Any \| None` indexing errors). See the override block in `pyproject.toml` for the canonical list + refresh procedure.
