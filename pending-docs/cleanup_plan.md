...
- **Library swaps explicitly skipped:** sqlglot (DuckDB's own `json_serialize_sql` is more correct for our SQL); alembic (overkill for single-file SQLite metadata DBs); Schemathesis (existing openapi.json → tsc check is enough for solo dev).
...
- **1.2** Add OTel and parallel-network dependencies: `opentelemetry-api`, `opentelemetry-sdk`, `opentelemetry-instrumentation-fastapi`, `opentelemetry-instrumentation-botocore`, `opentelemetry-instrumentation-aiohttp-client`, `structlog`, and `aiodns`. Configure SDK with console exporter for dev (Jaeger/Tempo can be added post-v2.0).
...
- **1.4** Migrate emitters: replace contextvar mirror + `_section_timings` dict appends with `tracer.start_as_current_span`. Replace scattered `logger.info("%s ...", a)` with `structlog.get_logger().info("event", a=a)`.
- **1.4a** **Refactor rDNS caching workflow (`rdns_cache.py`):** Re-architect the lookup flow to perform concurrent asynchronous DNS resolution using `aiodns` and `asyncio.gather()`. Execute bulk lookups in parallel outside of thread-blocking loops. Batch commit all resolved IP-host mappings in a single SQLite bulk transaction via `con.executemany` inside a `with con:` write lock block, rather than making a separate connection open/commit/close per resolved IP. Resolves the major sync-worker bottleneck where bulk IP lookups block for several minutes. See [design_rdns_async.md](design_rdns_async.md) for full design specification.
...
- **5b.3a** **Refactor programmatic Terraform generation (`backend/utils/terraform_gen.py`):** Replace custom programmatic f-string HCL construction and its regex-based `_hcl_escape` helper (prone to injection if complex strings bypass patterns) with standard `.tf.json` generation. Terraform natively supports JSON configurations with complete feature-parity. Python's standard `json.dumps()` guarantees robust escaping for all strings. This eliminates the custom-HCL escaping injection vector entirely, deletes 200+ LOC of fragile template code, and simplifies validation. See [design_terraform_json.md](design_terraform_json.md) for specification.
- **5b.3b** **Carve-up of `backend/core/metadata_db.py` (3,168 lines):**
  ...
  - `backend/core/metadata/base.py` (SQLite connection pooling, WAL mode, migrations executor, and database locks)
  ...
  Refactor `backend/core/metadata_db.py` into a backward-compatible shim using multi-inheritance composition to extend all concern sub-modules. See [design_metadata_carveup.md](design_metadata_carveup.md) for full design blueprint.
...
- **9b.9** **Test cleanup:** keep existing test coverage; rewrite imports as files move. Add coverage for any newly-extracted utility modules (wizard state machine, datatable column picker logic).
