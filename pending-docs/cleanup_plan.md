...
- **10.13** **Final mypy sweep:** every backend module either mypy-strict or explicitly justified in `pyproject.toml` overrides with a comment. Goal: `ignore_errors` list ≤ 3 modules max. ⚠️ **Partial 2026-06-11.** Replaced the five wildcard patterns (`backend.routers.*`, `backend.utils.*`, etc.) with an explicit per-file list of 36 modules that actually have errors today. Net effect: ~93 files that were silently swept up by the wildcards now get real mypy coverage — `_fos_cache` annotation, `FrameType | None` reassignment, and several other latent issues surfaced. New modules added to those packages no longer silently inherit `ignore_errors`. **Still pending:** the 36 explicit entries need burndown to reach the ≤ 3 target. Each is a per-module fix-or-annotate. See the override block in `pyproject.toml` for the canonical list + refresh procedure.
