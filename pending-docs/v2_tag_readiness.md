...
| 10.13 — `tool.mypy.overrides` ignore_errors list ≤ 3 modules | ≤ 3 | 10 explicit per-file entries | ⚠️ partial (2026-06-12) | Reduced 36 → 32 → 22 → 16 → 10 across four 2026-06-12 burndown waves (26 modules cleared total). Wave-4 cleared the 3-error tier: `config` (typed Any | None for `tracked_call` import-fallback), `deps` (typed `_pool_cm: Any | None` + `get_con` return as `Iterator`), `repositories/_base` (widened `actual_cols` to `list | set` + `live_res` annotation + `bucket_opt` narrowing), `repositories/sessions` (cast spec to `Any` for dual-shape support + renamed shadowed `literals` var), `routers/share_auth` (dropped buggy `iso_z_now() and 24*60*60` truthiness check + `or []` fallback), plus `repositories/insights/registry` (typed `__init__` / `register` returns). 10 remaining are the heaviest-error modules (`repositories/origin` 13, `routers/session_scoring_admin` 11). |
...
1. **10.13 burndown** — fix-or-annotate the 10 modules in the `tool.mypy.overrides` list. 2026-06-12 cleared 26 modules across four waves (zero / single / 1-2 / 3-error tiers). Remaining 10 are the 5-13 error tier: `repositories/origin` (13), `routers/session_scoring_admin` (11), `provision/fastly_api` (5 after my earlier signature fix landed), `routers/services/core` (7), `routers/admin` (7), `utils/remote_access` (7), `core/ingest` (7), `provision/orchestrator` (5), `routers/provision` (5), `utils/telemetry_proxy` (5). Target: list ≤ 3 entries.
...
- **10.13 burndown (10 remaining):** 26 cleared 2026-06-12 across four waves (zero, single, 1-2, and 3-error tiers). Remaining 10 are 5-13 errors each; the heaviest (`repositories/origin.py` with 13 `Any | None` indexing errors, `routers/session_scoring_admin` with 11) need bulk narrowing. Estimate: **1-2 hours** to clear or annotate-with-pragma all 10 down to ≤ 3.
