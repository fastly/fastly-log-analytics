...
| 10.14 — frontend `coverage.thresholds.lines=58` | 58 | gate 53, actual 55.19 | ❌ pending | Bumping the gate alone breaks CI; need ~120 more covered lines first. Low-coverage candidates: `lib/api/custom-fields.ts` (13%), `lib/workers/parseJson.ts` (0%), `lib/ui/toast.ts` (8%). |
...
3. **10.14 frontend coverage push** — write tests for `parseJson.ts`, `custom-fields.ts`, `toast.ts` (and any other sub-30% files). Once total clears 58%, ratchet `coverage.thresholds.lines=58`.
...
- **10.14 frontend coverage push:** ~120 covered lines. Three small files identified. Estimate: **1-2 hours**.
