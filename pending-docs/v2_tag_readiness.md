...
| 10.13 — `tool.mypy.overrides` ignore_errors list ≤ 3 modules | ≤ 3 | 22 explicit per-file entries | ⚠️ partial (2026-06-12) | Reduced from 5 wildcards → 36 → 32 → 22. 2026-06-12 burndown waves dropped 14 modules total: `log_fields`, `sqlite_profiler`, `sql_validator` (`_reject` → `NoReturn`), `vcl_validator` (builtin `callable` → `typing.Callable`), `duckdb` / `routers/dashboard` / `repositories/insights/definitions` / `provision/fos_setup` (`dict[str, Any]` annotations), `models/lake` (`# type: ignore[attr-defined]` for pyiceberg private API), `utils/telemetry` (typed `FrameType | None`), `utils/bot_sources` (declared the lru_cache wrapper's actual `tuple[dict, ...]` return), `utils/terraform_gen` + `provision/session_scoring_orchestrator` (widened `generate_capture_vcl` to accept `dict \| None` per the body's `(arg or {})` pattern), `routers/bootstrap` (annotated `PRESETS: dict[str, dict[str, Any]]`). 22 remaining are mostly multi-error modules needing real type narrowing (`repositories/origin` has 13 indexing errors, etc). |
...
1. **10.13 burndown** — fix-or-annotate the 22 modules in the `tool.mypy.overrides` list. 2026-06-12 cleared 14 modules (zero-error + single-error tier). Remaining 22 are mostly multi-error: `repositories/origin` (13 errors), `routers/admin` (7), `routers/services/core` (8), `provision/fastly_api` (10), `routers/session_scoring_admin` (11). Target: list ≤ 3 entries.
...
- **10.13 burndown (22 remaining):** 14 cleared 2026-06-12 (the zero-error / single-error tier). Remaining modules are 2-13 actual errors each; the heaviest (`repositories/origin.py` with 13 `Any | None` indexing errors, `routers/session_scoring_admin` with 11, `provision/fastly_api` with 10) need bulk narrowing. Estimate: **2-3 hours** to clear or annotate-with-pragma all 22 down to ≤ 3.
