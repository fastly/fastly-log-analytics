...
| 10.13 — `tool.mypy.overrides` ignore_errors list ≤ 3 modules | ≤ 3 | 32 explicit per-file entries | ⚠️ partial (2026-06-12) | Reduced from 5 wildcards → 36 → 32. 2026-06-12 burndown dropped `log_fields`, `sqlite_profiler`, `sql_validator` (typed `_reject` as `NoReturn`), `vcl_validator` (swapped builtin `callable` for `typing.Callable`). Each remaining entry needs a fix-or-annotate session. |
...
1. **10.13 burndown** — fix-or-annotate the 32 modules in the `tool.mypy.overrides` list (PR #9 made this trackable; 2026-06-12 cleared the 4 zero-error / trivially-typed modules). Target: list ≤ 3 entries.
...
- **10.13 burndown (32 remaining):** 4 cleared 2026-06-12 (the zero-error / 1-2-error tier). Remaining modules are 5-40 actual errors each; some (`repositories/origin.py` with 13 `Any | None` indexing errors) need deeper attention. Estimate: **3-5 hours** to clear or annotate-with-pragma all 32 down to ≤ 3.
