...
function DashboardBody({
  ...
  // Composite /api/dashboard/bundle returns aggregates + security/top-bots
  // in one round-trip and seeds the existing cache keys. The aggregates
  // useQuery below reads from the seeded cache; the bot-card useQuery
  // does the same. Cold load = 1 backend request (was 2-3 before the
  // lazy-followup pattern was removed).
  ...
  const { data: aggregates, isLoading: isLoadingAggs, isFetching: isFetchingAggs } = useServiceQuery(
    ['dashboard', 'aggregates', activeServiceId, startTime, endTime, filterPayload, metric, config.effectiveInterval, undefined],
    async ({ signal }) => {
