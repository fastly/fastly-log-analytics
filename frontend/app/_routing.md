...
**Filter URL codec:** all `ReportLayout` pages plus `/query` and `/sessions` emit the modern `?filters=<json>` form on store mutations (write path owned by `useFilterUrlSync`). On INGRESS every page also accepts the legacy `?filter_<col>=<val>` short form (read path owned by `hydrateFilterStoreFromUrl`) — drill-down emitters (`FilterValueCell`, `session-urls`, `AlertsList`) still produce those URLs, and any external bookmarks pinned to that shape keep working. The legacy form gets normalized to `?filters=` on the first store touch.
