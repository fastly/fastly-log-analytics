...
export default async function RootLayout({
  ...
  if (bootstrap) {
    ...
    if (sid) {
      ...
      // Always seed ['last-sync', sid] — even when null — so the cache
      // entry exists before SyncStatusBadge's useLastSync() subscribes.
      // Without a pre-existing entry, useQuery's getOptimisticResult calls
      // QueryCache.build → notify mid-render → "Cannot update a component
      // while rendering a different component" (webkit-flaky a11y failure).
