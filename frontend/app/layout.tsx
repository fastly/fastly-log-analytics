...
// force-dynamic is REQUIRED now that the layout SSR-fetches
// /api/bootstrap (see fetchBootstrapServerSide below) using cookies()
// from next/headers. Without it, Next.js can statically generate
// layout.html at build time when the backend isn't reachable —
// shipping an empty HTML shell, the SSR fetch never runs at request
// time, and the React Query cache stays unseeded.
//
// The earlier removal of force-dynamic (2026-06-06) was about
// avoiding an SSR roundtrip per navigation when there was no
// per-request work to do. Now there IS per-request work: SSR'ing
// bootstrap puts the share banner and dependent caches into the
// first HTML paint, eliminating the cold client-side RTT from every
// page visit. The same ~100-300 ms per nav we were avoiding is now
// the work we ACTUALLY want to do server-side.
