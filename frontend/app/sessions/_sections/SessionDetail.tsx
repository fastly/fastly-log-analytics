      ...
      <DialogContent className="max-w-6xl max-h-[85vh] flex flex-col p-4 md:p-6 overflow-hidden">
        <DialogHeader className="shrink-0 mb-2">            <DialogTitle className="flex items-center gap-2 text-base">
          ...
          <DialogDescription className="sr-only">
            Detailed metadata and per-request records for the selected client session.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-auto min-h-0 flex flex-col">
          {isErrorDetail && (
            <div
              role="alert"
              className="mb-3 flex flex-col items-start gap-2 rounded-md border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive"
            >
              <div className="font-medium">Failed to load session detail.</div>
              <div className="font-mono text-xs opacity-80 break-all">{extractApiError(detailError)}</div>
              <button
                type="button"
                onClick={() => { void refetchDetail() }}
                className="mt-1 rounded border border-destructive/50 px-2 py-1 text-xs hover:bg-destructive/20"
              >
                Retry
              </button>
            </div>
