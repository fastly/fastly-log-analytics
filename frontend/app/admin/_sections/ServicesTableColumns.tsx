...
export function buildServiceColumns(deps: ServiceColumnDeps): ColumnDef<ServiceConfig>[] {
  ...
  return [
    {
      accessorKey: 'name',
      // Plain-label header: the DataTable's StaticTableHeader already wraps
      // sortable columns in the sort <button> + arrow. Returning a <Button>
      // here nested a button inside that button — invalid HTML that the
      // browser restructures on parse, breaking hydration (React #418) — and
      // double-fired the sort toggle.
      header: 'Service Name',
      size: 200,
      cell: ({ row }) => (
        <div className="flex items-center gap-2 font-medium">
          {row.original.service_id === activeServiceId && (
            <Badge variant="default" className="h-5 px-1.5 text-[10px] uppercase font-bold bg-blue-600 hover:bg-blue-700 shadow-none border-none">Active</Badge>
          )}
          {row.getValue('name')}
        </div>
      )
