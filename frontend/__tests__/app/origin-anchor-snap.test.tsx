...
// Covers the fix for the SSR/client anchor-mismatch flash on /origin — see
// dashboard-anchor-snap.test.tsx for the full rationale. Renders the REAL
// ReportLayout (unlike origin.test.tsx's shallow smoke test) so the actual
...
vi.mock('@/app/origin/_sections/Aggregates', () => ({ Aggregates: () => <div data-testid="aggregates" /> }))
vi.mock('@/app/origin/_sections/Timeseries', () => ({ Timeseries: () => <div data-testid="timeseries" /> }))
vi.mock('@/app/origin/_sections/LatencyHeatmap', () => ({ LatencyHeatmap: () => <div data-testid="latency-heatmap" /> }))
...
test('first-paint aggregates query anchors on the snapped (stale) log extents, not "now"', async () => {
  ...
  const call = vi.mocked(apiLib.client.POST).mock.calls.find(([url]: any[]) => String(url).includes('/api/origin/aggregates'))
