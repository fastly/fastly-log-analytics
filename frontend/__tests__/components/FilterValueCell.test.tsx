...
describe('FilterValueCell', () => {
  ...
  // The DropdownMenu trigger + items in this component use base-ui's
  // pointer-event pipeline. `userEvent.click()` synthesises a full
  // pointer chain but jsdom omits PointerEvent's coordinate fields,
  // which base-ui interprets as an "outside" click and immediately
  // closes the popover before the menu item paint. `fireEvent.click`
  // bypasses the pointer-event layer and just dispatches `click`,
  // which is what the menu actually listens for. Keep these on
  // `fireEvent.click` until base-ui ships a jsdom-friendly pointer
  // path. Matches the convention documented in R-14 ("leave fireEvent
  // when the testid / synthetic event is the only viable hook").
  ...
  it('cmd/ctrl-click on the cell bypasses the menu and calls addFilter directly', () => {
    ...
    // mouseDown with modifier flags is the actual contract the component
    // listens to (the cmd-click shortcut handler reads `e.metaKey` on
    // mousedown to short-circuit the popover open). userEvent emits a
    // full pointer chain that doesn't expose modifier flags on mouseDown,
    // so fireEvent stays here intentionally — keep the keyboard-modifier
    // event raw rather than synthesizing a click.
  ...
  it('plain click on the cell still opens the menu', () => {
    render(<FilterValueCell filters={[{ column: 'pop', value: 'JFK' }]} />)
    const trigger = screen.getByRole('button', { name: /Filter actions for JFK/i })
    // Same base-ui caveat as above — fireEvent.click is the pragmatic
