...
// on Next-click. Each returns null on valid input, an error string otherwise.
function validateBucketName(v: string): string | null {
  if (!v) return null
  // RFC 1123-ish: lowercase letters, digits, hyphens; 3-63 chars; no leading/
  // trailing hyphen; no consecutive dots. FOS rejects names that fail this.
  if (v.length < 3 || v.length > 63) return 'Bucket name must be 3–63 chars.'
  if (!/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/.test(v)) {
    return 'Bucket name must be lowercase alphanumeric with hyphens (no leading/trailing -).'
  }
  return null
}
function validateAccessKey(v: string): string | null {
  if (!v) return null
  // AWS access-key IDs are 20 chars starting with AKIA (long-lived) or ASIA
  // (temporary session). FOS keys follow the same pattern.
  if (!/^(AKIA|ASIA)[A-Z0-9]{16}$/.test(v.trim())) {
    return 'Access key should start with AKIA or ASIA and be 20 chars total.'
  }
  return null
}
function validateS3Uri(v: string): string | null {
  if (!v) return null
  // Optional field — only validate when populated.
  if (!/^s3:\/\/[^/]+\/.+/.test(v.trim())) {
    return 'Expected an s3://bucket/key URI.'
  }
  return null
}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onBlur={(e) => setError('fos_bucket_name', validateBucketName(e.target.value))}
                aria-invalid={fieldErrors.fos_bucket_name != null}
                className="h-9 font-mono text-sm"
                placeholder="e.g. my-service-logs"
              />
              {fieldErrors.fos_bucket_name && (
                <p role="alert" className="text-[11px] text-destructive">{fieldErrors.fos_bucket_name}</p>
              )}
            </div>
            <div className="space-y-1.5">
              <LabelWithInfo
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onBlur={(e) => setError('iceberg_uri', validateS3Uri(e.target.value))}
              aria-invalid={fieldErrors.iceberg_uri != null}
              className="h-9 font-mono text-xs"
              placeholder="s3://bucket/iceberg/default/logs/metadata/..."
            />
            {fieldErrors.iceberg_uri && (
              <p role="alert" className="text-[11px] text-destructive">{fieldErrors.iceberg_uri}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onBlur={(e) => setError('fos_access_key', validateAccessKey(e.target.value))}
                aria-invalid={fieldErrors.fos_access_key != null}
                className="h-9 font-mono text-sm"
                placeholder="e.g. AKIA..."
              />
              {fieldErrors.fos_access_key && (
                <p role="alert" className="text-[11px] text-destructive">{fieldErrors.fos_access_key}</p>
              )}
            </div>
            <div className="space-y-1.5">
              <LabelWithInfo
