...
/**
 * The data-bound body of the Add-Filter dialog. Split out of
 * AddFilterDialog so it is only mounted once the dialog is opened
 * (``{open && <AddFilterDialogContent .../>}``).
 *
 * Why: this body calls ``useLogFieldsCatalog()`` and ``useFieldValues()``
 * during render. When those hooks mount a NEW React Query observer for an
 * uncached key, React Query's synchronous ``QueryCache.notify`` drives a
 * setState in the already-subscribed sibling observers (e.g.
 * SyncStatusBadge) mid-render → "Cannot update a component while rendering
 * a different component". Deferring the subtree behind ``open`` moves that
 * first subscription into a click-driven render pass (outside the sibling's
 * render), so FilterBar's initial render never mounts a new observer. The
 * trigger Button lives in the parent so it renders without the data hooks.
 */
export function AddFilterDialogContent({ onClose }: { onClose: () => void }) {
  // PII: masking analysts can't filter by client IP, so the field list drops
  // the IP-family entries and the default field falls back to `url`.
  const maskIps = useMaskIps()
  const [field, setField] = useState(maskIps ? 'url' : 'ip')
  const [value, setValue] = useState('')
  const [mode, setMode] = useState<'include' | 'exclude'>('include')
  const debouncedValue = useDebounce(value)

  const { data: catalog } = useLogFieldsCatalog()
  const commonFields = COMMON_FIELD_IDS
    .filter(id => !(maskIps && isIpFamilyField(id)))
    .map(id => ({
      value: id,
  ...
  const { data: suggestions, isLoading: isLoadingSuggestions } = useFieldValues({
    ...
    limit: 10,
    // The body only mounts while the dialog is open, so the prior
    // ``enabled: open && …`` gate collapses to the search-length check.
