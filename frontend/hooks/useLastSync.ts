 ...
 * ``useCronRunsStream`` sees a ``task === 'sync'`` event arrive —
...
export function useLastSync() {
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const e = (data as { entries?: Array<Record<string, unknown>> } | undefined)?.entries?.[0]
      return {
        started_at: (e?.started_at as string | undefined) ?? null,
        status: (e?.status as string | undefined) ?? null,
        duration_s: (e?.duration_s as number | undefined) ?? null,
      } as LastSyncInfo
    },
    ...
    // useCronRunsStream coalesces real completion events. The 60 s staleTime +
