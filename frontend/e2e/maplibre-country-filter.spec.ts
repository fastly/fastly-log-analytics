...
test('dashboard renders country geometry through the MapLibre worker', async ({ page }, testInfo) => {
  ...
  const prewarmCanvas = page.locator('canvas.maplibregl-canvas').first().waitFor({ state: 'attached', timeout: 45_000 })
  const fallbackText = page.getByText(/Interactive map unavailable/i).waitFor({ state: 'visible', timeout: 45_000 })
  await Promise.any([prewarmCanvas, fallbackText])
