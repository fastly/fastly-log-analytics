...
def test_backfill_skips_built_hours(tmp_path):
    ...
    _write_hour_pop_health(str(cache_root), f"{yday}-11", [{"pop": "IAD", "requests": 100, "errors": 10, "cache_hits": 90, "bandwidth_bytes": 1000, "p50_rtt_us": 10.0, "p95_ttfb_ms": 100.0}])
    ...
    for d_iso in (day_a, day_b):
        for h in range(24):
            _write_hour_pop_health(
                ...
                f"{d_iso}-{h:02d}",
                [
                    {"pop": "IAD", "requests": 50, "errors": 0, "cache_hits": 50, "bandwidth_bytes": 500, "p50_rtt_us": 10.0, "p95_ttfb_ms": 100.0},
                ],
            )
...
def test_compact_writes_per_day_file_with_correct_sums(tmp_path):
    ...
    for h in range(24):
        _write_hour_pop_health(
            ...
            f"{day}-{h:02d}",
            [
                {"pop": "IAD", "requests": 100, "errors": 5, "cache_hits": 80, "bandwidth_bytes": 1000, "p50_rtt_us": 10.0, "p95_ttfb_ms": 50.0},
                {"pop": "LHR", "requests": 50, "errors": 0, "cache_hits": 45, "bandwidth_bytes": 500, "p50_rtt_us": 20.0, "p95_ttfb_ms": 100.0},
            ],
        )
...
def test_compact_skips_active_day(tmp_path):
    ...
    for h in range(24):
        _write_hour_pop_health(
            str(cache_root),
            f"{today}-{h:02d}",
            [{"pop": "IAD", "requests": 5, "errors": 0, "cache_hits": 5, "bandwidth_bytes": 50, "p50_rtt_us": 10.0, "p95_ttfb_ms": 100.0}],
        )
