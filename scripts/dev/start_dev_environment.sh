...
# Define generic connection commands with fallbacks
REMOTE_STANDARD_FORWARD_CMD="${REMOTE_STANDARD_FORWARD_CMD:-gcloud compute ssh fastly-log-analysis --project=se-development-9566 --zone=us-central1-a -- -N -L 3001:127.0.0.1:3000 -L 8001:127.0.0.1:8000}"
REMOTE_HIGH_SCALE_FORWARD_CMD="${REMOTE_HIGH_SCALE_FORWARD_CMD:-kubectl port-forward svc/frontend-svc -n se-demo 3002:3000 & kubectl port-forward svc/backend-svc -n se-demo 8002:8000}"
