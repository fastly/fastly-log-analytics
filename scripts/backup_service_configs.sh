...
#   BACKUP_MARKER_VM_PATH     (optional) On-VM path that maps INTO the backend's
#                             data dir, e.g. /mnt/app-data/system/last_config_backup.json
#                             (host side of the container's /app/data/system/...).
#                             On success the script writes a small JSON marker
#                             there; the admin health snapshot reads it and shows
#                             config-backup freshness (SRE-11). Unset → the health
#                             endpoint reports config_backup=null ("never recorded").
#
...
# Dry-run must be side-effect-free, so describe the plan and exit BEFORE the
# ssh leg below — `gcloud compute ssh ... sudo tar` is a real, authenticated
# side effect on the production VM, not something a dry run should perform.
if [ "${DRY_RUN}" -eq 1 ]; then
  echo "[backup] DRY RUN: would 'gcloud compute ssh ${BACKUP_INSTANCE} --zone=${BACKUP_ZONE}' + 'sudo tar' ${BACKUP_CONFIGS_DIR}, then upload to ${GCS_PATH}"
  exit 0
fi
...
# (Dry-run already exited above, before the ssh leg.)
...
# 4. (Optional) Drop a VM-local freshness marker the backend's health snapshot
#    reads (SRE-11) so the operator can see "is my only irreplaceable state
#    backed up, and how stale?" without leaving the admin UI. Only runs when
#    BACKUP_MARKER_VM_PATH is set (operator-supplied → no infra paths in the
#    repo). Written via sudo because the data dir is typically root-owned; tee
#    creates it 0644 so the container's app user can read it.
if [ -n "${BACKUP_MARKER_VM_PATH:-}" ]; then
  NOW_ISO=$(date -u +%Y-%m-%dT%H:%M:%SZ)
  MARKER_JSON="{\"at\":\"${NOW_ISO}\",\"remote\":\"${GCS_PATH}\"}"
  echo "[backup] writing freshness marker -> ${BACKUP_INSTANCE}:${BACKUP_MARKER_VM_PATH}"
  gcloud compute ssh "${BACKUP_INSTANCE}" --zone="${BACKUP_ZONE}" --command="\
    set -e; \
    sudo mkdir -p \$(dirname '${BACKUP_MARKER_VM_PATH}'); \
    printf '%s' '${MARKER_JSON}' | sudo tee '${BACKUP_MARKER_VM_PATH}' >/dev/null"
fi
echo "[backup] done."
