...
def test_time_series_rebuckets_minute_counts_to_a_wider_interval() -> None:
    store = AggregateStore()
    store.apply(
        batch(
            "b1",
            "request",
            [
                {"event_id": "1", "timestamp": "2026-09-01T00:00:10Z"},
                {"event_id": "2", "timestamp": "2026-09-01T00:04:59Z"},
                {"event_id": "3", "timestamp": "2026-09-01T00:05:00Z"},
            ],
        )
    )

    series = store.time_series("svc", "request", bucket_seconds=300)

    assert series == (
        (datetime(2026, 9, 1, 0, 0, tzinfo=UTC), 2),
        (datetime(2026, 9, 1, 0, 5, tzinfo=UTC), 1),
    )


def test_time_series_rejects_bucket_not_a_multiple_of_a_minute() -> None:
    store = AggregateStore()

    with pytest.raises(ValueError):
        store.time_series("svc", "request", bucket_seconds=45)


def test_time_series_filters_to_requested_window() -> None:
    store = AggregateStore()
    store.apply(
        batch(
            "b1",
            "request",
            [
                {"event_id": "1", "timestamp": "2026-09-01T00:00:00Z"},
                {"event_id": "2", "timestamp": "2026-09-01T00:10:00Z"},
            ],
        )
    )
    ...
    series = store.time_series(
        ...
        end=datetime(2026, 9, 1, 0, 20, tzinfo=UTC),
    )

    assert series == ((datetime(2026, 9, 1, 0, 10, tzinfo=UTC), 1),)
...
def test_high_cardinality_dimension_reports_approximate_once_capacity_exceeded() -> None:
    store = AggregateStore(heavy_hitter_capacity=2)
    store.apply(
        batch(
            "b1",
            "request",
            [{"event_id": str(i), "url": f"/page-{i}"} for i in range(5)],
        )
    )

    response = store.response("svc", "request")

    assert response.exact is False
    assert response.approximation_error is not None
    assert len(response.top_values) <= 2


def test_low_cardinality_dimension_stays_exact_under_capacity() -> None:
    store = AggregateStore(heavy_hitter_capacity=2)
    store.apply(batch("b1", "request", [{"event_id": "1", "url": "/one"}, {"event_id": "2", "url": "/one"}]))

    response = store.response("svc", "request")

    assert response.exact is True
    assert response.approximation_error is None
    assert response.top_values == (("/one", 2),)
