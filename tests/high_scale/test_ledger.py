...
def test_mark_source_missing_records_terminal_status_and_error(ledger: HighScaleLedger) -> None:
    ledger.discover("svc", "request", "raw/request/a.gz", "sha256:a")
    ledger.claim("svc", "raw/request/a.gz", "worker")
    ledger.mark_source_missing("svc", "raw/request/a.gz", error="NoSuchKey")
    source = ledger.source("svc", "raw/request/a.gz")
    assert source.status == "source_missing"
    assert source.last_error == "NoSuchKey"


def test_mark_source_missing_is_idempotent(ledger: HighScaleLedger) -> None:
    ledger.discover("svc", "request", "raw/request/a.gz", "sha256:a")
    ledger.claim("svc", "raw/request/a.gz", "worker")
    ledger.mark_source_missing("svc", "raw/request/a.gz", error="NoSuchKey")
    ledger.mark_source_missing("svc", "raw/request/a.gz", error="NoSuchKey (retry)")
    # Second call is a no-op — the first recorded error is preserved rather
    # than clobbered by a later duplicate report.
    assert ledger.source("svc", "raw/request/a.gz").last_error == "NoSuchKey"


def test_mark_source_missing_refuses_to_override_acknowledged_source(ledger: HighScaleLedger) -> None:
    ledger.discover("svc", "request", "raw/request/a.gz", "sha256:a")
    claim = ledger.claim("svc", "raw/request/a.gz", "worker")
    ledger.mark_appended("svc", "raw/request/a.gz", claim.lease_generation)
    ledger.mark_archived("svc", "raw/request/a.gz", claim.lease_generation, "manifest-1", 1)
    ledger.acknowledge("svc", "raw/request/a.gz", "manifest-1")
    with pytest.raises(ArchiveNotVerified):
        ledger.mark_source_missing("svc", "raw/request/a.gz", error="NoSuchKey")


def test_mark_source_missing_requires_an_error_description(ledger: HighScaleLedger) -> None:
    ledger.discover("svc", "request", "raw/request/a.gz", "sha256:a")
    ledger.claim("svc", "raw/request/a.gz", "worker")
    with pytest.raises(ValueError):
        ledger.mark_source_missing("svc", "raw/request/a.gz", error="")
