...
def test_rollup_heal_marks_rollup_coverage_ready_under_durable_mode(monkeypatch, stub_source, stub_progress):
    """C2 (final whole-branch review): a successful heal pass calls
    backfill_missing_hour_bundles — the SAME real coverage-establishing
    function main.py's startup catch-up uses — so it's the correct job to
    (re-)mark durable-mode rollup coverage ready, self-healing a failed or
    timed-out startup catch-up within the hour."""
