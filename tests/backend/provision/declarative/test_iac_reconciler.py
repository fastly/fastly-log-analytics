...
def mock_config_dir(tmp_path, monkeypatch):
    ...
    # Patch Path inside iac_reconciler so config loading maps to tmp_path / configs / {id}.json
    def mock_path(path_str):
        if "configs/" in path_str:
            parts = path_str.split("/")
            return configs_dir / parts[-1]
...
class TestIaCReconciler:
    ...
    @patch("backend.provision.declarative.iac_reconciler.is_mock_mode", return_value=True)
    def test_verify_readiness_probe_skips_in_mock_mode(self, mock_is_mock):
        """Verify readiness probe completes immediately in mock mode."""
        ...
        status_cb.assert_called_with("🔍 [Mock Mode] Skipping active HTTP readiness probe for Compute Scorer.")

    @patch("httpx.get")
    @patch("backend.provision.declarative.iac_reconciler.is_mock_mode", return_value=False)
    @patch("time.sleep")
    def test_verify_readiness_probe_retries_and_succeeds(self, mock_sleep, mock_is_mock, mock_get):
