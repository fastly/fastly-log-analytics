...
def test_dispatch_rum_minute_discovers_the_rum_prefix_not_the_regular_one():
    with (
        ...
        result = ingest.dispatch_rum_minute.apply(args=(SERVICE_ID, "raw/rum/2026/08/27/14/05/")).get()
    assert result == 2
    mock_rum.assert_called_once_with(SERVICE_ID, prefix_subpath="raw/rum/2026/08/27/14/05/")
...
def test_convert_rum_task_converts_a_beacon_file_not_a_regular_log_file():
    with (
        ...
        result = ingest.convert_rum.apply(args=(SERVICE_ID, "raw/rum/b.json.gz")).get()
    assert result == "committed"
    assert mock_rum.call_args.args[:2] == (SERVICE_ID, "raw/rum/b.json.gz")
