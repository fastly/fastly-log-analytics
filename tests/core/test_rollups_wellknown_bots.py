...
def test_read_falls_back_when_time_range_exceeds_366_days():
    """Finding 007: read_wellknown_bots_rollup loops one entry per hour
    between start_dt and end_dt with no upper bound, so an attacker who
    can influence the time range can force the reader to allocate
    millions of dict entries for a multi-year span. Guard: spans wider
    than 366 days short-circuit to None (live SQL has its own row-count
    bounds) before the hour loop runs."""
    from backend.core import rollups
