...
async def await_until(
    ...
    Use this in `pytest.mark.asyncio` tests where you need to wait on a
    condition without blocking the event loop — e.g. waiting until an
    SSE subscriber's queue is actually registered before publishing.

    The previous pattern (``await asyncio.sleep(0)`` once) only yielded a
    single tick: if the consumer task hadn't reached ``q.get()`` by then,
    a subsequent publish raced past the registration and was dropped.
    Polling on ``pub.subscriber_count(svc) >= 1`` is deterministic.
    """
    import asyncio
